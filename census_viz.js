...
function drawBarChart(data, title) {
    ...
    // Add sorting functionality using Partition Sort (Quick Sort)
    function sortBars() {
        let sortedData = data.slice(); // Create a copy of the data
        ...
        function quickSortStep(arr, low, high) {
            if (low < high) {
                ...
                setTimeout(() => {
                    quickSortStep(arr, low, pi - 1);
                    quickSortStep(arr, pi + 1, high);
                }, 1000);
            }
        }

        quickSortStep(sortedData, 0, sortedData.length - 1); // Start the sorting process
    }

    // Trigger the sorting when the bars are drawn
    sortBars();
}
